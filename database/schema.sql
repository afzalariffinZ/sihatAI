...
-- Weight entries table (alias for weights for compatibility)
CREATE VIEW public.weight_entries AS SELECT * FROM public.weights;

-- Food database table (for common foods - optional)
...
CREATE INDEX idx_meals_datetime ON public.meals(datetime);
CREATE INDEX idx_meals_user_datetime ON public.meals(user_id, datetime);
CREATE INDEX idx_weights_user_id ON public.weights(user_id);
CREATE INDEX idx_weights_date ON public.weights(date);
...
CREATE INDEX idx_activity_entries_date ON public.activity_entries(date);
CREATE INDEX idx_activity_entries_user_date ON public.activity_entries(user_id, date);
...
-- RLS Policies for weights table
...
-- RLS Policies for step_data table
...
-- RLS Policies for activity_entries table
...
-- RLS Policies for foods table (public read, admin write)
